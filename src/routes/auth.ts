...
authRoutes.get('/apple', async (c) => {
  const state = generateToken();
  const redirectUri = `${c.env.SITE_URL}/auth/apple/callback`;

  setCookie(c, 'oauth_state', state, {
    path: '/',
    httpOnly: true,
    secure: true,
    sameSite: 'None', // Apple uses form_post which is a cross-site POST
    maxAge: 600,
  });

  const authUrl = getAppleAuthUrl(c.env.APPLE_CLIENT_ID, redirectUri, state);
  return c.redirect(authUrl);
});

// Apple uses form_post — callback is POST
authRoutes.post('/apple/callback', async (c) => {
  const body = await c.req.parseBody();
  const code = body['code'] as string | undefined;
  const state = body['state'] as string | undefined;
  const storedState = getCookie(c, 'oauth_state');
  const userDataRaw = body['user'] as string | undefined;
  ...
  try {
    ...
    const clientSecret = await generateAppleClientSecret(
      ...
      c.env.APPLE_KEY_ID,
      c.env.APPLE_CLIENT_SECRET // This is the private key
    );
    ...
    if (!decoded.email) {
      return c.html(loginPage('Could not retrieve email from Apple. Please try again.'));
    }

    // Apple sends user info only on first authorization
    let userName = decoded.email.split('@')[0];
    if (userDataRaw) {
      try {
        ...
        // User data parse failed, use email-derived name
      }
    }
    ...
    // created_pending
    const newMember = await c.env.DB.prepare('SELECT * FROM members WHERE email = ?')
      .bind(decoded.email.toLowerCase())
      .first();
    ...
    return c.html(loginPage('Something went wrong with Apple sign-in. Please try again.'));
  }
});
